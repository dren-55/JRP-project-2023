class Commands {
    ...
    async isWebElementEnabled(locator) {
        ...
        */
        ...
        async waitForAttributeValue(selector, attributeName, value, timeout) {
            browser.waitUntil(
              () => $(selector).getAttribute(attributeName) === value,
              {
                timeout,
                timeoutMsg: `Element '${selector}' did not reach attribute '${attributeName}' value '${value}' after ${timeout}ms`,
                interval: 100,
              },
            );
          }
